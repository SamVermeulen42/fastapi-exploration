...
async def websocket_endpoint_download(id: int, websocket: WebSocket, db: Session = Depends(get_db)):
...
async def websocket_endpoint_activate(id: int, websocket: WebSocket, db: Session = Depends(get_db)):
    await websocket.accept()
